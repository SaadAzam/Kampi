...
The mobile and desktop lobby now uses optimized supplied artwork, RPS art, source-faithful metallic frames and navigation, reduced-motion-aware CSS animations, and live catalog/balance/leaderboard data. The design-fidelity correction restores the original goalkeeper, compact two-column mobile cards, supplied right-aligned Play buttons and Battle base, and removes invented promotional headings and copy. Existing account, history and game-session behavior is preserved, with improved startup and transient-failure recovery.

See [lobby implementation and validation](lobby-redesign-2026-09-10.md) for asset budgets, provenance and the 34 passing Chromium/WebKit browser checks. The earlier Railway source-snapshot failure has a separate [deployment recovery change](railway-snapshot-fix-2026-09-10.md). These changes remain local for owner review and commit. Earlier historical gap lists in this document should be read alongside the later dated deployment and demo reports.
